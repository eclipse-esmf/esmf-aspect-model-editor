...
describe('Test load different characteristics', () => {
  ...
  context('TestCode', () => {
    it('can add LengthConstraint', () => {
      cy.clickAddTraitPlusIcon('TestCode').then(() => {
        cy.dbClickShape('Constraint1').then(() => {
          cy.get(FIELD_constraintName).click({force: true}).get('mat-option').contains('LengthConstraint').click({force: true});
          cy.get('[data-cy="minValue"]').type('1').click({force: true});
          cy.get('[data-cy="maxValue"]').type('10').click({force: true});
  ...
  context('TestMeasurement', () => {
    it('can modify unit', () => {
      cy.dbClickShape('TestMeasurement').then((shape: mxgraph.mxCell) => {
        cy.get('[data-cy=clear-unit-button]').click({force: true});
        cy.get(FIELD_unit).clear().type('amperePerM').get('mat-option').contains('amperePerMetre').click({force: true});
        cy.get(SELECTOR_editorSaveButton)
          .focus()
          .click({force: true})
          .then(() =>
            cy.getUpdatedRDF().then(rdf => {
              expect(rdf).to.contain('bamm-c:unit unit:amperePerMetre');
            })
          );
      });
    });
    ...
    it('can modify dataType', () => {
      cy.dbClickShape('TestMeasurement').then((shape: mxgraph.mxCell) => {
        ...
        cy.get(FIELD_dataType)
          .clear()
          .type('double')
          .get(FIELD_dataTypeOption)
          .eq(0)
          .click({force: true})
          .then(() => {
            cy.get(SELECTOR_editorSaveButton)
              ...
              .then(() =>
                cy.getUpdatedRDF().then(rdf => {
                  expect(rdf).to.contain('bamm:dataType xsd:double');
                })
              );
          });
      });
    ...
    it('can add FixedPointConstraint', () => {
      cy.clickAddTraitPlusIcon('TestMeasurement').then(() => {
        cy.dbClickShape('Constraint2').then(() => {
          ...
          cy.get('[data-cy="scale"]').type('1').click({force: true});
          cy.get('[data-cy="integer"]').type('1').click({force: true});
          cy.get(SELECTOR_editorSaveButton)
            .focus()
            .click({force: true})
            .then(() => {
    ...
    it('can not enter incorrect value for scale', () => {
      cy.dbClickShape('Constraint2')
        ...
        .then(() => {
          cy.get('[data-cy="scale"]')
            .clear()
            .type('-1')
            .then(() => {
              cy.get('[data-cy="integer"]')
                .click({force: true})
                .then(() => {
    ...
    it('can modify scale', () => {
      cy.dbClickShape('Constraint2').then((shape: mxgraph.mxCell) => {
        cy.get('[data-cy="scale"]')
          .clear()
          .type('10')
          .click({force: true})
  ...
  context('TestQuantifiable', () => {
    it('can add EncodingConstraint', () => {
      cy.clickAddTraitPlusIcon('TestQuantifiable').then(() => {
        cy.dbClickShape('Constraint3').then(() => {
          cy.get(FIELD_constraintName).click({force: true}).get('mat-option').contains('EncodingConstraint').click({force: true});
          cy.get(SELECTOR_editorSaveButton)
            .focus()
            .click({force: true})
            .then(() => {
  ...
  context('TestSortedSet', () => {
    it('can add RangeConstraint', () => {
      cy.clickAddTraitPlusIcon('TestSortedSet').then(() => {
        cy.dbClickShape('Constraint5').then(() => {
          cy.get(FIELD_constraintName).click({force: true}).get('mat-option').contains('RangeConstraint').click({force: true});
          cy.get('[data-cy="maxValue"]').type('100');
          cy.get('[data-cy="minValue"]').type('1');
          cy.get(SELECTOR_editorSaveButton)
            .focus()
            .click({force: true})
            .then(() => {
  ...
  context('TestCollection', () => {
    it('can add LocaleConstraint', () => {
      cy.clickAddTraitPlusIcon('TestCollection').then(() => {
        cy.dbClickShape('Constraint6').then(() => {
          cy.get(FIELD_constraintName).click({force: true}).get('mat-option').contains('LocaleConstraint').click({force: true});
          cy.get(FIELD_localeCode).clear().type('de-D').get('mat-option').contains('de-DE').click({force: true});
          cy.get(SELECTOR_editorSaveButton)
            .focus()
            .click({force: true})
            .then(() => {
  ...
  context('TestDuration', () => {
    it('can add LanguageConstraint', () => {
      cy.clickAddTraitPlusIcon('TestDuration').then(() => {
        cy.dbClickShape('Constraint7').then(() => {
          cy.get(FIELD_constraintName).click({force: true}).get('mat-option').contains('LanguageConstraint').click({force: true});
          cy.get(FIELD_languageCode).clear().type('English').get('mat-option').contains('en').click({force: true});
          cy.get(SELECTOR_editorSaveButton)
            .focus()
            .click({force: true})
            .then(() => {
