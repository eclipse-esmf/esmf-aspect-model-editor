...
import {Aspect, DefaultEntity, DefaultQuantifiable, Trait} from '@bame/meta-model';
...
describe('Test load external reference with same namespace', () => {
  it('Loading property element with there children from external file with same namespace', () => {
    ...
    cy.intercept(
      {
        ...
        headers: {'Bame-Model-Urn': 'io.openmanufacturing.digitaltwin:1.0.0:external-property-reference-with-children.txt'},
      },
      {
    ...
    cy.fixture('/external-reference/same-namespace/model-with-external-property-reference-with-children')
      ...
      .then((aspect: Aspect) => {
        ...
        expect((<DefaultEntity>entity.properties[0].property.characteristic.dataType).name).to.equal('ChildrenEntity2');
      })
  ...
  it('Loading operation element with there children from external file with same namespace', () => {
    ...
    cy.intercept(
      {
        method: 'GET',
        url: 'http://localhost:9091/bame/api/models',
        headers: {'Bame-Model-Urn': 'io.openmanufacturing.digitaltwin:1.0.0:external-operation-reference-with-children.txt'},
      },
      {
    ...
    cy.fixture('/external-reference/same-namespace/model-with-external-operation-reference-with-children')
      ...
      .then((aspect: Aspect) => {
        ...
        const entity1 = <DefaultEntity>aspect.operations[0].input[0].property.characteristic.dataType;
        ...
        const entity2 = <DefaultEntity>aspect.operations[0].input[1].property.characteristic.dataType;
        ...
        const entity3 = <DefaultEntity>aspect.operations[0].output.property.characteristic.dataType;
    ...
    cy.intercept(
      {
        method: 'GET',
        url: 'http://localhost:9091/bame/api/models',
        headers: {'Bame-Model-Urn': 'io.openmanufacturing.digitaltwin:1.0.0:external-characteristic-reference-with-children.txt'},
      },
      {
  ...
  it('Loading custom unit element from external file with same namespace', () => {
    cy.intercept('POST', 'http://localhost:9091/bame/api/models/validate', {fixture: 'model-validation-response.json'});
    cy.intercept('GET', 'http://localhost:9091/bame/api/models/namespaces', {
      'com.bosch.nexeed.digitaltwin:1.0.0': ['external-unit-reference.txt'],
    });
    ...
    cy.intercept(
      {
        method: 'GET',
        url: 'http://localhost:9091/bame/api/models',
        headers: {'Bame-Model-Urn': 'com.bosch.nexeed.digitaltwin:1.0.0:external-unit-reference.txt'},
      },
      {
    ...
    cy.intercept(
      {
        method: 'GET',
        url: 'http://localhost:9091/bame/api/models',
        headers: {'Bame-Model-Urn': 'io.openmanufacturing.digitaltwin:1.0.0:external-entity-reference-with-children.txt'},
      },
      {
  ...
  it('Loading different elements from several external files with same namespace', () => {
    ...
    cy.intercept(
      {
        method: 'GET',
        url: 'http://localhost:9091/bame/api/models',
        headers: {'Bame-Model-Urn': 'io.openmanufacturing.digitaltwin:1.0.0:external-trait-reference.txt'},
      },
      {
    ...
    cy.fixture('/external-reference/same-namespace/model-with-several-external-reference')
      ...
      .then((aspect: Aspect) => {
        ...
        expect((<DefaultEntity>aspect.properties[0].property.characteristic.dataType).name).to.equal('ExternalEntity');
      })
      .then(() => cy.getUpdatedRDF())
      .then(rdf => {
