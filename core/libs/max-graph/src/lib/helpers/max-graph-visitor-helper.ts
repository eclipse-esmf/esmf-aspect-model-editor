...
export class MaxGraphVisitorHelper {
  ...
  static getModelInfo(modelElement: NamedElement, file: NamespaceFile): ModelBaseProperties {
    const loadedFile = MaxGraphHelper.injector ? MaxGraphHelper.injector.get(LoadedFilesService) : null;

    try {
      const [currentNamespace] = file.rdfModel.getAspectModelUrn().replace('urn:samm:', '').split(':');
      const [, elementNamespace] = MaxGraphHelper.getNamespaceFromElement(modelElement);
