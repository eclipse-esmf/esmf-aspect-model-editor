...
@Component({
  selector: 'bci-right-input-field',
...
export class RightInputFieldComponent extends InputFieldComponent<DefaultEither> implements OnInit, OnDestroy {
  ...
  setRightControl() {
    ...
    this.filteredCharacteristicTypes$ = this.initFilteredCharacteristicTypes(this.rightControl, this.metaModelElement.aspectModelUrn)
      .pipe(map(charList => charList.filter(char => char.urn !== this.parentForm.get('leftCharacteristic')?.value?.aspectModelUrn)));
  }
  ...
  createNewCharacteristic(characteristicName: string) {
    ...
    if (this.metaModelElement.aspectModelUrn === urn || this.parentForm.get('name').value === characteristicName) {
      this.notificationsService.error('Element right cannot link itself');
      this.rightControl.setValue('');
      return;
    }

    if (characteristicName === this.parentForm.get('leftCharacteristic')?.value?.name) {
      this.notificationsService.error('Element right cannot point to the same characteristic as the left element.');
      this.rightControl.setValue('');
      return;
    }
