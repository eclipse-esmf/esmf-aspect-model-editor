...
import {BaseMetaModelElement, DefaultCharacteristic, Unit, DefaultConstraint} from '@bame/meta-model';
...
export abstract class InputFieldComponent<T extends BaseMetaModelElement> implements OnDestroy, OnChanges {
  ...
  isAsteriskVisible(formControlName: string): boolean {
    return this.parentForm.get(formControlName)?.hasValidator(Validators.required);
  }
