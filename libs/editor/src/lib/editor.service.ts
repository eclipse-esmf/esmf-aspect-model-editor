...
import {AlertService, AlertType} from '@bci-web-core/core';
...
import {ModelApiService, ModelValidatorService} from '@bame/api';
...
export class EditorService {
  ...
  loadNewAspectModel(rdfAspectModel: string, isDefault?: boolean) {
    ...
    return this.rdfService.loadModel(rdfAspectModel).pipe(
      switchMap(rdfModel =>
        this.validateCurrentModel(rdfModel, rdfAspectModel).pipe(
          tap(() => {
  ...
  private validateCurrentModel(rdfModel: RdfModel, rdfAspectModel: string) {
    return this.getIdentifiedModelValidator(rdfModel, rdfAspectModel).pipe(
      ...
      catchError(error => {
        if (error.status === 0) {
          return of(this.notificationsService.error('Bosch Aspect Model Editor Service is Down', null, null, 10000));
        }
        return of(
  ...
  private loadCurrentModel(rdfModel: RdfModel, rdfAspectModel: string, validationResOfLoadedRdfModel: any) {
    return this.modelService.loadRdfModel(rdfModel, rdfAspectModel, validationResOfLoadedRdfModel).pipe(
      ...
      catchError(error => {
        ...
        return of(this.notificationsService.error('Error on loading the aspect model'));
      })
    );
  }

  // This validation is needed so that old models can be migrated to the new OMP format.
  private getIdentifiedModelValidator(rdfModel: RdfModel, rdfAspectModel: string) {
    const serializedModel = this.rdfService.serializeModel(rdfModel);

    if (rdfModel.BAMM().identifier.includes(Bamm.OLD_BASE_URI)) {
      return this.modelApiService.getOldValidationErrors(rdfAspectModel);
    }
  ...
  openAlertBox() {
    this.alertService.openAlertBox({
      type: AlertType.None,
      title: 'Aspect Model is missing',
      description: 'To start modelling, please create a new or load an existing model first. After that you can add new node types',
