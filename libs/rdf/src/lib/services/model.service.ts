...
import {Aspect, BaseMetaModelElement, DefaultConstraint, LoadedAspectModel} from '@bame/meta-model';
import {InstantiatorService} from '@bame/instantiator';
import {environment} from 'environments/environment';
import {CachedFile, NamespacesCacheService} from '@bame/cache';
import {Bamm} from '@bame/vocabulary';
import {ModelApiService} from '@bame/api';
import {RdfService} from './rdf.service';
import {LogService, AppConfig, SaveValidateErrorsCodes, NotificationsService} from '@bame/shared';
...
export class ModelService {
  ...
  loadRdfModel(rdfModel: RdfModel, rdfAspectModel: string, errors: Array<any>): Observable<Aspect> {
    // TODO maybe add previous if external reference have the same BAMM version ...
    if (this.currentCachedFile) {
      this.currentCachedFile.reset();
    }
    const bammVersion: string = rdfModel.BAMM().version;
    const isOldBammVersion = rdfModel.BAMM().identifier.includes(Bamm.OLD_BASE_URI);

    try {
      if (bammVersion < this.config.oldMinBammVersion && isOldBammVersion) {
        return throwError(() => `BAMM version ${bammVersion} is too small to migrate.`);
      }

      if (bammVersion > this.config.oldMaxBammVersion && isOldBammVersion) {
        return throwError(() => `The provided Aspect Model is using BAMM version ${bammVersion} which is too high.`);
      }

      if (bammVersion < this.config.minBammVersion && !isOldBammVersion) {
        return throwError(() => `BAMM version ${bammVersion} is too small to migrate.`);
      }

      if (bammVersion > this.config.currentBammVersion && !isOldBammVersion) {
        return throwError(
          () => `The provided Aspect Model is using BAMM version ${bammVersion} which is too high.
            BAME is currently based on BAMM ${this.config.currentBammVersion}.`
        );
      }

      let rdfModelObservable = of(rdfModel);
      if ((bammVersion < this.config.currentBammVersion && bammVersion >= this.config.minBammVersion) || isOldBammVersion) {
  ...
  private migrateAspectModel(bammVersion: string, rdfAspectModel: string, errors: Array<any>): Observable<RdfModel> {
    this.notificationsService.info(
      `Migrating from DTS ${bammVersion} BAMM version to OMP ${this.config.currentBammVersion} BAMM version`,
      null,
      null,
      5000
    );
    ...
    return this.modelApiService.migrateAspectModel(rdfAspectModel, errors).pipe(
      ...
      tap(() =>
        this.notificationsService.info(
          `Successfully migrated from DTS ${bammVersion} BAMM version to OMP ${this.config.currentBammVersion} BAMM version`,
          null,
          null,
          5000
        )
