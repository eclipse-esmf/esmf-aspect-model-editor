...
h| Workaround
a|
Check that you have not missed parts of the TTL file. Also check against syntax errors consulting https://eclipse-esmf.github.io/samm-specification/2.2.0/namespaces.html[Namespaces and versions (SAMM 2.2.0)^,opts=nofollow].
|===
...
https://eclipse-esmf.github.io/samm-specification/2.2.0/datatypes.html[Datatypes (SAMM 2.2.0)^,opts=nofollow]
...
Fixed for Aspect Model Editor 2.2.0 and higher: If the backend is not available, a message is shown to the user.
|===
...
h| Explanation
a| This happens due to autoformat not being enabled and/or the hierarchical layout being disabled.

h| Workaround
a|
One of the following options can be used to fix the problem:

. Right click on the imported element, select the option "Select all references" and drag the elements to a free space.
. Manually select the desired elements and drag them to a free space.
. Switch to hierarchical layout from settings and:
.. (autoformat disabled) Click format button from toolbar. All elements will be rearranged without overlapping.
.. (autoformat enabled) When importing from workspace, the format happens automatically and the elements will not overlap. In case of overlapping, click format button.
|===
...
NOTE: This issue is caused by the Aspect Model Editor not being notarized or signed with an Apple Developer certificate. See xref:support/known-limitations.adoc#current-known-limitations-macos[Current limitations on macOS] for background information.
...
h| Explanation
a|
If your macOS version is running Apple's Gatekeeper security feature, it could happen that the application is started in a kind of sandbox. You can easily detect this by reading the following in the JavaScript error message: `Uncaught Exception: Error: spawn /private/var...` (see image below).

image::error-mac-javascript-main-process.png[width=50%]

h| Workaround
a|
The keyword here is `/private/var`, which means that your application is quarantined and can only be unlocked by you. For this you must open Terminal and execute the following command:
...
Please keep in mind that this is not an official Apple-signed version, and it must be confirmed in the security settings first. You may be asked several times before the app opens.
...
* Go to *System Settings* and click on *Privacy & Security*.
* Scroll down to the security section and click *Open Anyway*, entering your admin password if prompted.
